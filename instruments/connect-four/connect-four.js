  ...
  if (!document.getElementById("c4-styles")) {
    ...
    style.textContent = `
      ...
      .c4-card {
        ...
        background: #fff;
        border-radius: 16px;
        padding: clamp(18px, 6vmin, 32px);
        text-align: center;
        box-shadow: 0 4px 16px rgba(0,0,0,0.12);
      }
      .c4-card h2 { margin: 0 0 8px; font-size: clamp(19px, 5vmin, 24px); font-weight: 700; }
      .c4-card p { margin: 0 0 clamp(14px, 5vmin, 28px); color: #6b7280; }
      .c4-btn {
      ...
      .c4-header {
        ...
        margin-bottom: clamp(8px, 3vmin, 20px); gap: 8px;
      }
      ...
      .c4-logo-card { width: 100%; max-width: min(260px, 60vmin); margin: 0 auto 12px; display: block; }
      .c4-logo-header { width: 100%; max-width: min(170px, 40vmin); margin: 0 auto; display: block; }
      .c4-players {
        display: flex; align-items: center; justify-content: center;
        gap: clamp(10px, 4vmin, 24px); margin-bottom: clamp(8px, 3vmin, 20px); flex-wrap: wrap;
      }
      .c4-player {
        display: flex; align-items: center; gap: 10px; padding: clamp(8px, 2.5vmin, 12px) clamp(10px, 3vmin, 16px);
      ...
      .c4-status { text-align: center; margin-bottom: clamp(8px, 3vmin, 20px); min-height: 0; }
      .c4-status-box { padding: clamp(8px, 2.5vmin, 14px); border-radius: 10px; font-size: clamp(14px, 3.6vmin, 18px); font-weight: 700; }
      ...
      .c4-board {
        background: #fff; border-radius: 18px; padding: clamp(8px, 2.5vmin, 20px);
        box-shadow: 0 4px 16px rgba(0,0,0,0.15); border: clamp(2px, 0.6vmin, 4px) solid #000;
        max-width: 100%; box-sizing: border-box;
      }
      /* Cell size scales with the smaller of viewport width/height (vmin), so the whole
         7x6 board -- plus header, players and status above it -- always fits on screen,
         in portrait or landscape, without ever needing to shrink below a usable tap target. */
      .c4-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: clamp(4px, 1.6vmin, 8px); }
      .c4-col { display: flex; flex-direction: column; gap: clamp(4px, 1.6vmin, 8px); }
      .c4-cell {
        width: clamp(28px, 8vmin, 52px); height: clamp(28px, 8vmin, 52px); border-radius: 50%;
        border: clamp(2px, 0.9vmin, 4px) solid #000;
      ...
      /* Short viewports (phones in landscape, small laptops in a tight window): trim the
         chrome around the board rather than the board itself, since vertical space is the
         scarce resource here. */
      @media (max-height: 480px) {
        .c4-root { padding: 6px; }
        .c4-title p { display: none; }
        .c4-logo-header { max-width: min(120px, 22vh); }
        .c4-header { margin-bottom: 6px; }
        .c4-players { margin-bottom: 6px; gap: clamp(8px, 3vmin, 16px); }
        .c4-player { padding: 6px 10px; }
        .c4-status { margin-bottom: 6px; }
        .c4-cell { width: clamp(24px, 11vh, 52px); height: clamp(24px, 11vh, 52px); }
      }
    `;
