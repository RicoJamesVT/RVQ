...
const MINIGAME_ACTIONS = {
  ...
  // shop's `minigames` list), right alongside Gator Grooves. Same "full
  // standalone web app, not a canvas mini-game" shape as chess/beatbot/
  // organ/mini golf/blackbook/Gator Grooves/Vinyl Snake/Bayou Break
  // Station/VT Dirt/Gator Jam Slam/Rico1200/Rico's Mini DAW above (own DOM/
  // iframe overlay, bundled locally -- its own Three.js scene reusing the
  // same vendored lib/three.min.js the rest of the game already ships, no
...
function createCrateDigging3DGame() {
  ...
  function fireOutcomeFX(mesh, outcome) {
    ...
    if (outcome.type === 'rare') {
      ...
      fx.popup(`+${outcome.pts}`, screenPos.x, screenPos.y, { color: '#e0b040', size: 24 });
      ...
      fx.shake(0.012, 0.1);
      fx.popup('DUD', screenPos.x, screenPos.y, { color: '#8a8090', size: 15 });
    }
  }
...
window.addEventListener('keydown', (e) => {
  ...
  if (!keys[k]) {
    ...
    if (k === 'g') {
      ...
      if (state === 'play') openPhotoMode();
      else if (state === 'photo') state = photoReturnState;
    }
    if (k === 'escape' && state === 'photo') { state = photoReturnState; }
...
// out on the deck (see the `minigames` list below); `opts.crates` is left
// empty since there's nothing to dig for out here.
...
const shops = {
  ...
  swampfood: makeShop('swampfood', {
    ...
    //
    // Dig Dash cabinet -- a cozy 3D crate-digging endless runner, mirrored
    // on the opposite side of the open floor at (3,6), same clearance
    // logic as Gator Grooves above (clear of the counter table (row 3),
...
// [G] Photo Mode -- same "return state" pattern as hotkeysReturnState/
// crateReturnState above, though in practice it's only ever opened from
// 'play'. photoCamOffset is a small screen-space nudge (see the 'photo'
// branch in update() and the ctx.translate() in render()) that lets the
// player frame a cleaner shot; it resets to centered every time the mode
// is (re)opened.
let photoReturnState = 'play';
let photoCamOffset = { x: 0, y: 0 };
const PHOTO_NUDGE_STEP = 6, PHOTO_NUDGE_MAX = 36;
function openPhotoMode() {
  if (state !== 'play') return;
  photoReturnState = state;
  photoCamOffset = { x: 0, y: 0 };
  state = 'photo';
}
// [T] Turbo Taunt / Trick Button -- purely cosmetic, see triggerTrick()
// below. `trick` drives a brief spin on the player sprite (drawPlayer());
// `trickSparkles` is a small bounded particle burst, same "push, cap,
// shift the oldest off" shape as the skate trail near the top of this file
// so it can never grow unbounded no matter how fast the key is mashed.
let trick = null; // { t, dur }
let trickSparkles = [];
const TRICK_NAMES = ['KICKFLIP!', '360 SPIN!', 'OLLIE!', 'BOARDSLIDE!'];
function triggerTrick() {
  if (state !== 'play' || !player.skating) return;
  trick = { t: 0, dur: 0.5 };
  for (let i = 0; i < 8; i++) {
    const ang = Math.random() * Math.PI * 2;
  ...
  toast = { text: TRICK_NAMES[Math.floor(Math.random() * TRICK_NAMES.length)], t: 0.9 };
  music.trickChime();
}
function updateTrickSparkles(dt) {
  for (let i = trickSparkles.length - 1; i >= 0; i--) {
    const p = trickSparkles[i];
    p.life -= dt;
    if (p.life <= 0) { trickSparkles.splice(i, 1); continue; }
    p.x += p.vx * dt;
    p.y += p.vy * dt;
    p.vy += 70 * dt; // slight gravity so sparkles arc instead of flying dead straight
  }
}
function drawTrickSparkles() {
  for (const p of trickSparkles) {
    const k = Math.max(0, p.life / p.maxLife);
    ctx.globalAlpha = k;
    ctx.fillStyle = '#fff6d6';
    ctx.beginPath();
    ctx.arc(p.x, p.y, 1.5 + 2 * k, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.globalAlpha = 1;
}
...
const music = {
  ...
  // [T] Turbo Taunt sparkle cue -- a quick, tiny ascending chime, distinct
  // from (and much shorter than) sting() so it reads as a light flourish
  // rather than the bigger "moment" sting() is used for elsewhere.
  trickChime() {
    if (!this.ctx) return;
    const t = this.ctx.currentTime + 0.02;
    [76, 81, 86].forEach((n, i) => this.note(t + i * 0.045, 'square', n, 0.08, 0.055, 0.05));
  },
};
...
function createHyperSwimOverlay() {
  ...
  style.textContent = `
    ...
    #ricoHyperSwimApp.open { display: flex; }
    #ricoHyperSwimApp .rhs-bar {
      flex: 0 0 auto; display: flex; align-items: center; justify-content: space-between;
      gap: 12px; padding: 10px 14px;
      background: linear-gradient(#23104a, #050014);
      border-bottom: 2px solid #a3004a;
      padding-top: calc(10px + env(safe-area-inset-top, 0px));
    }
    #ricoHyperSwimApp .rhs-title {
      color: #f4ecd8; font: bold 14px monospace; letter-spacing: 0.5px;
      white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    }
    #ricoHyperSwimApp .rhs-close {
      flex: 0 0 auto; cursor: pointer;
      background: rgba(163,0,74,0.25);
      border: 1.5px solid rgba(255,61,127,0.85);
      color: #f4ecd8; border-radius: 8px;
      padding: 7px 16px; font: bold 13px monospace;
      -webkit-user-select: none; user-select: none;
    }
    #ricoHyperSwimApp .rhs-close:active { background: rgba(163,0,74,0.5); }
    #ricoHyperSwimApp iframe {
      flex: 1 1 auto; width: 100%; border: 0; background: #000;
    }
  `;
  document.head.appendChild(style);

  hyperSwimOverlayEl = document.createElement('div');
  hyperSwimOverlayEl.id = 'ricoHyperSwimApp';

  const bar = document.createElement('div');
  bar.className = 'rhs-bar';
  const title = document.createElement('div');
  title.className = 'rhs-title';
  title.textContent = "HYPER SWIM '96 DELUXE";
  const closeBtn = document.createElement('div');
  closeBtn.className = 'rhs-close';
  closeBtn.textContent = '\u2190 BACK TO THE POOL DECK';
  bindTap(closeBtn, closeHyperSwimApp);
  bar.appendChild(title);
  bar.appendChild(closeBtn);

  hyperSwimOverlayFrame = document.createElement('iframe');
  hyperSwimOverlayFrame.setAttribute('allow', 'autoplay');

  hyperSwimOverlayEl.appendChild(bar);
  hyperSwimOverlayEl.appendChild(hyperSwimOverlayFrame);
  document.body.appendChild(hyperSwimOverlayEl);
}
createHyperSwimOverlay();

// Opens the Hyper Swim overlay and switches state to 'hyperSwimApp'. Called
// from MINIGAME_ACTIONS.hyperswim (E on the sign on the deck, or tapping
// it), same entry points every other mini-game uses.
function openHyperSwimApp() {
  hyperSwimReturnState = state;
...
// Gator Grooves. Same "full-screen DOM overlay with an <iframe>" trick as
// chess/the beat bot/the organ/mini golf/the blackbook/Gator Grooves above.
//
// Ships as a bundled, self-contained page at instruments/dig-dash/index.html
// -- the exact same local-file pattern CHESS_APP_URL/BEAT_BOT_APP_URL/
// ORGAN_APP_URL/MINI_GOLF_APP_URL/BLACKBOOK_APP_URL/CROC_SWAMP_APP_URL use.
// Unlike Gator Grooves/Vinyl Snake (plain 2D canvas, no library needed),
// Dig Dash's scene is genuinely 3D, so it points its <script> tag at the
// same '../../lib/three.min.js' the main game already vendors locally for
// its own 3D mini-games (see loadThreeJS() above) instead of fetching
// three.js from a CDN -- one shared local copy, no network call, so it
// loads and plays the same with or without a connection, same as every
// other bundled app here.
const DIG_DASH_APP_URL = 'instruments/dig-dash/index.html';
let digDashOverlayEl = null, digDashOverlayFrame = null;
let digDashReturnState = 'play';
let digDashHistoryPushed = false; // mirrors labHistoryPushed/.../crocSwampHistoryPushed -- see openDigDashApp()/closeDigDashApp()

function createDigDashOverlay() {
  const style = document.createElement('style');
  style.textContent = `
    #digDashApp {
      position: fixed; inset: 0; z-index: 1000;
    ...
    #digDashApp.open { display: flex; }
    #digDashApp .dd-bar {
      flex: 0 0 auto; display: flex; align-items: center; justify-content: space-between;
      gap: 12px; padding: 10px 14px;
      background: linear-gradient(#3a2e5c, #1f1734);
      border-bottom: 2px solid #ffd166;
      padding-top: calc(10px + env(safe-area-inset-top, 0px));
    }
    #digDashApp .dd-title {
      color: #fff6e9; font: bold 14px monospace; letter-spacing: 0.5px;
      white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    }
    #digDashApp .dd-close {
      flex: 0 0 auto; cursor: pointer;
      ...
      color: #fff6e9; border-radius: 8px;
      padding: 7px 16px; font: bold 13px monospace;
      -webkit-user-select: none; user-select: none;
    }
    #digDashApp .dd-close:active { background: rgba(255,209,102,0.4); }
    #digDashApp iframe {
      flex: 1 1 auto; width: 100%; border: 0; background: #000;
    }
  `;
  ...
  digDashOverlayEl = document.createElement('div');
  digDashOverlayEl.id = 'digDashApp';

  const bar = document.createElement('div');
  bar.className = 'dd-bar';
  const title = document.createElement('div');
  title.className = 'dd-title';
  title.textContent = 'DIG DASH';
  const closeBtn = document.createElement('div');
  closeBtn.className = 'dd-close';
  closeBtn.textContent = '\u2190 BACK TO SWAMP FOOD';
  bindTap(closeBtn, closeDigDashApp);
  bar.appendChild(title);
  bar.appendChild(closeBtn);

  digDashOverlayFrame = document.createElement('iframe');
  digDashOverlayFrame.setAttribute('allow', 'autoplay');

  digDashOverlayEl.appendChild(bar);
  digDashOverlayEl.appendChild(digDashOverlayFrame);
  document.body.appendChild(digDashOverlayEl);
}
createDigDashOverlay();

// Opens the Dig Dash overlay and switches state to 'digDashApp'. Called
// from MINIGAME_ACTIONS.digdash (E on the cabinet, or tapping its floating
// sign), same entry points every other mini-game uses.
function openDigDashApp() {
  digDashReturnState = state;
  digDashOverlayFrame.src = DIG_DASH_APP_URL;
  digDashOverlayEl.classList.add('open');
  state = 'digDashApp';
  // Same throwaway-history-entry trick as openInstrument()/openChessApp()/
  // .../openCrocSwampApp() above, so the browser/OS back gesture closes
  // the Dig Dash overlay instead of leaving the game entirely.
  history.pushState({ ricoDigDashApp: true }, '');
  digDashHistoryPushed = true;
}
...
function update(dt) {
  ...
  if (state === 'splash') {
    ...
    updateTrickSparkles(dt);
  } else if (state === 'photo') {
    // [G] Photo Mode: player movement is simply frozen (no movePlayer()
    // call here, same as every other non-'play' state) -- arrows/WASD
    // instead nudge the screen-space camera offset applied in render().
    // selectMove/menuMove are the same edge-triggered -1/0/1 globals the
    // menus above consume; PHOTO_NUDGE_MAX keeps the nudge "slight" per
    // the feature's intent.
    if (selectMove) {
      photoCamOffset.x = Math.max(-PHOTO_NUDGE_MAX, Math.min(PHOTO_NUDGE_MAX, photoCamOffset.x + selectMove * PHOTO_NUDGE_STEP));
      selectMove = 0;
    }
    if (menuMove) {
      photoCamOffset.y = Math.max(-PHOTO_NUDGE_MAX, Math.min(PHOTO_NUDGE_MAX, photoCamOffset.y + menuMove * PHOTO_NUDGE_STEP));
      menuMove = 0;
    }
    // Sparkles from a trick triggered just before entering photo mode
    // should still finish playing out rather than freezing mid-air.
    updateTrickSparkles(dt);
    ...
    // 'minigolfApp'/'blackbookApp'/'crocSwampApp'/'vinylSnakeApp'/
    // 'bayouBreakApp'/'gatorJamSlamApp'/'vtDirtApp' just above: the DOM
    // overlay (see createDigDashOverlay()) owns input while Dig Dash is
    // loaded -- its own close button and [Esc] handle closing it directly.
    // buyPressed is still consumed here too so the on-screen [X] touch
...
function render(time) {
  ...
  ctx.save();
  // [G] Photo Mode: a small screen-space nudge applied before every other
  // world transform below, so it shifts the fully-composited scene (tiles,
  // buildings, player, everything) by a few pixels without touching any of
  // the actual camera/zoom math those transforms do -- see openPhotoMode()/
  // the 'photo' branch in update() for how photoCamOffset gets set.
  if (state === 'photo') ctx.translate(photoCamOffset.x, photoCamOffset.y);
  if (map.outside) {
  ...
  // this point.
  // [G] Photo Mode: HUD, the [E]/[X] action pill, and toasts all stay
  // hidden for as long as state === 'photo' -- see drawPhotoModeHint()
  // below for the one small, low-opacity reminder of how to get back.
  if (state !== 'photo') {
    drawHUD();
    drawLabBeacon(time);
  }
  ...
  if (state === 'photo') drawPhotoModeHint();
  if (state === 'minigame' && activeMinigame) activeMinigame.draw();
  if (toast && state !== 'photo') drawToast();
}
...
function drawPlayer(time) {
  ...
  // [T] Turbo Taunt: while `trick` is active, spin the whole sprite about
  // its own center for the duration of the animation -- purely visual,
  // doesn't touch player.x/y/dir or any collision/movement code at all.
  const trickSpin = trick ? (trick.t / trick.dur) * Math.PI * 2 * (player.dir === 'left' ? -1 : 1) : 0;
  if (trickSpin) {
    ctx.save();
    const pivotX = player.x, pivotY = footY - SPR_H / 2 - (player.skating ? 4 : 0) + bob;
    ctx.translate(pivotX, pivotY);
    ctx.rotate(trickSpin);
    ctx.translate(-pivotX, -pivotY);
  }
...
// [G] Photo Mode's one on-screen element: a small, low-opacity corner
// label so the player still knows how to get back out, without it reading
// as a "prompt" competing with the clean shot they opened this mode for.
function drawPhotoModeHint() {
  ctx.textAlign = 'right';
  ctx.fillStyle = 'rgba(244,236,216,0.45)';
  ctx.font = 'bold 12px monospace';
  ctx.fillText('PHOTO MODE \u2014 [\u2190\u2191\u2192\u2193] NUDGE \u00b7 [G] EXIT', VIEW_W - 10, VIEW_H - 10);
  ctx.textAlign = 'left';
}
...
function drawHotkeysPopup() {
  ...
  const rows = [
    ...
    ['B', 'skateboard on & off'],
    ['T', 'trick (while skating, just for fun)'],
